...
## xUtils3 的导入, 需要在MyApplication类中初始化

>使用Gradle  compile 'org.xutils:xutils:3.3.36'
		
1. 导入一个module,必然报错(插件找不到).将下面这句代码添加Project的build.gradle中(dependencies方法中如果已存在直接添加里面的代码就行了)
...
2. 打开Project Structure 添加依赖

3. 在项目中初始化,必须新建类MyApplication继承自Application,在onCreate()方法中初始化

			  x.Ext.init(this);
			//x.Ext.setDebug(BuildConfig.DEBUG);

4. 不要忘记给上面的MyApplication配置name(在清单文件的Application节点中)
